...
use rovo::IntoNestRouter;
use rovo::{Router, aide::openapi::OpenApi, axum::IntoApiResponse, routing::get, rovo};
...
use crate::routes::fs_router;
use crate::{
    config::Config,
    models::{FilePath, FsFile, FsListDirectory, PaginationParams},
    state::AppState,
};
use dotenvy;
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let app = Router::new()
        .nest(
            ...
            // find / replace
            // copy / paste
            // format <-- maybe not, this should be implemented somewhere else
            fs_router(state.clone()),
        )
