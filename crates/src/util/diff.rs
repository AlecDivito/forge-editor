...
pub fn diff(old: &str, new: &str) -> Option<ChangeRange> {
    ...
    while prefix_lines < old_lines.len()
        ...
        && old_lines[prefix_lines] == new_lines[prefix_lines] // guards a hash collision
    {
        prefix_lines += 1;
    }
    ...
    while suffix_lines < max_suffix
        && old_hashes[old_lines.len() - 1 - suffix_lines] == new_hashes[new_lines.len() - 1 - suffix_lines]
        && old_lines[old_lines.len() - 1 - suffix_lines] == new_lines[new_lines.len() - 1 - suffix_lines]
    {
        suffix_lines += 1;
    }

    let old_span_start: usize = old_lines[..prefix_lines].iter().map(|l| l.len()).sum();
    let old_span_end = old.len() - old_lines[old_lines.len() - suffix_lines..].iter().map(|l| l.len()).sum::<usize>();
    let new_span_start: usize = new_lines[..prefix_lines].iter().map(|l| l.len()).sum();
    let new_span_end = new.len() - new_lines[new_lines.len() - suffix_lines..].iter().map(|l| l.len()).sum::<usize>();

    let (char_prefix, char_suffix) =
        char_prefix_suffix(&old[old_span_start..old_span_end], &new[new_span_start..new_span_end]);
...
fn char_prefix_suffix(a: &str, b: &str) -> (usize, usize) {
    ...
    let prefix_len = a_chars.get(prefix).map(|(i, _)| *i).unwrap_or(a.len().min(b.len()));

    let max_suffix = (a_chars.len() - prefix).min(b_chars.len() - prefix);
    let mut suffix = 0;
    while suffix < max_suffix && a_chars[a_chars.len() - 1 - suffix].1 == b_chars[b_chars.len() - 1 - suffix].1 {
        suffix += 1;
    }
    let suffix_len = if suffix == 0 { 0 } else { a.len() - a_chars[a_chars.len() - suffix].0 };
...
pub fn byte_offset_to_position(text: &str, byte_offset: usize, encoding: &str) -> LspPosition {
    ...
    for ch in text[..byte_offset].chars() {
        if ch == '\n' {
            ...
            character += if encoding == "utf-8" { ch.len_utf8() as u32 } else { ch.len_utf16() as u32 };
        }
    }
