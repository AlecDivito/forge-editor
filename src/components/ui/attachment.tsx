...
function AttachmentDescription({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      ...
      className={cn("mt-0.5 block max-w-full min-w-0 truncate text-xs text-muted-foreground", className)}
      {...props}
    />
  );
}
...
function AttachmentActions({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "relative z-20 flex shrink-0 items-center group-data-[orientation=vertical]/attachment:absolute group-data-[orientation=vertical]/attachment:right-3 group-data-[orientation=vertical]/attachment:top-3",
        className,
      )}
      {...props}
    />
...
function AttachmentAction({ className, variant, size = "icon", ...props }: React.ComponentProps<typeof Button>) {
  return (
    <Button
...
function AttachmentTrigger({ className, render, type, ...props }: useRender.ComponentProps<"button">) {
  return useRender({
    ...
    props: mergeProps<"button">(
      { type: render ? type : (type ?? "button"), className: cn("absolute inset-0 z-10 outline-none", className) },
      props,
    ),
    render,
...
function AttachmentGroup({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div data-slot="attachment-group" className={cn("flex min-w-0 gap-3 overflow-x-auto py-1", className)} {...props} />
  );
}
