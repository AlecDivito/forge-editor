...
const createTokClasses = (theme: string, highlightStyles: CreateThemeOptions["styles"]) => {
    ...
    highlightStyles.forEach(({ tag, color, fontWeight, fontStyle, textDecoration }) => {
        const tagNames = Array.isArray(tag) ? tag.map((t) => t.name) : [tag]; // Handle arrays
...
export const requestHoverToolTip = async (view: EditorView, pos: number /* side:  -1 | 1 */) => {
    ...
    if (range) {
        // TODO: Maybe we can highlight what we are doing.
        // LSP lines are 0-indexed; CodeMirror's .line() is 1-indexed — the
        // +1 here was missing for any range past line 0 in the original.
